...
class PlaneMesh extends Mesh {
  ...
  PlaneMesh(String id, {num width: 1.0, num height: 1.0, int subdivisions: 1, bool ground: false})
      : super(id) {
    ...
    for (var row = 0; row <= subdivisions; row++) {
      for (var col = 0; col <= subdivisions; col++) {
        ...
        positions.addAll([px.toDouble(), py, -pz.toDouble()]);
  ...
  _createFromHighMap(num width, num height, int subdivisions, num minHeight, num maxHeight, html.ImageData imageData, int bufferWidth, int bufferHeight) {
